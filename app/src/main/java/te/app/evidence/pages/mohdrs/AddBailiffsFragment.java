...
import te.app.evidence.databinding.FragmentAddBailiffsBinding;
...
public class AddBailiffsFragment extends BaseFragment {

    private Context context;
    @Inject
    AddBailiffsViewModel viewModel;
    FragmentAddBailiffsBinding binding;

    @Nullable
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_add_bailiffs, container, false);
    ...
    private void setEvent() {
        viewModel.liveData.observe(((LifecycleOwner) context), (Observer<Object>) o -> {
            ...
            if (Constants.CASE_CLIENTS_CATEGORIES.equals(((Mutable) o).message)) {
                ...
                toastMessage(((AddMohdrResponse) mutable.object).mMessage);
                MovementHelper.finishWithResult(new PassingObject(((AddMohdrResponse) mutable.object).getReportersData()), context);
            }
        });
    }
    ...
    private void showCategories() {
        PopUpMenuHelper.showCategoriesPopUp(context, binding.inputCat, viewModel.getCaseClientsCategoriesData().getCategories()).
                setOnMenuItemClickListener(item -> {
                    ...
                    viewModel.getAddMohdrRequest().setCatid(viewModel.getCaseClientsCategoriesData().getCategories().get(item.getItemId()).getId());
                    return false;
                });
    }
            ...
            if (bundle != null && bundle.containsKey(Constants.BUNDLE)) {
                ...
                if (resultCode == Constants.CLIENTS_CODE) {
                    ...
                    if (clientsResponse.getCounter() != 0) {
                        viewModel.getAddMohdrRequest().setMokelText(getString(R.string.client_selected));
                        binding.inputClients.setText(getString(R.string.client_selected));
                    } else {
                        viewModel.getAddMohdrRequest().setMokelText(null);
                    ...
                    if (clientsResponse.getCounter() != 0) {
                        viewModel.getAddMohdrRequest().setKhesmText(getString(R.string.khesm_selected));
                        binding.inputKhesm.setText(getString(R.string.khesm_selected));
                    } else {
                        viewModel.getAddMohdrRequest().setKhesmText(null);
